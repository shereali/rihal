...
3. CRUD pages for backend-backed operational modules (homework, lesson plans, hostel, transport, HR, property). **Homework, lesson plans, hostel rooms, and transport routes are now implemented** (`/homework`, `/homework/:id`, `/lesson-plans`, `/hostel`, `/transport`) and linked in the role-aware sidebar.
