<template>
  <div class="page-wrapper">
    <div class="page-header-row">
      <div>
        <h1>একাডেমিক বিভাগ</h1>
        <p class="page-subtitle">বিভাগ, শিক্ষক-বিষয় বরাদ্দ ও সিট তৈরি</p>
      </div>
      <div class="header-actions">
        <button class="btn btn-primary" @click="showForm = !showForm">
          <Icon name="plus" class="btn-icon" /> নতুন বিভাগ
        </button>
      </div>
    </div>
    ...
    <div class="filter-bar card">
      <div class="filter-row">
        <div class="search-box">
          <Icon name="search" />
          <input v-model="search" placeholder="বিভাগ খুঁজুন..." @keyup.enter="load" />
        </div>
    ...
    <div v-else-if="!sections.length" class="empty-card">
      <div class="empty-icon"><Icon name="academic" /></div>
      <h3>এখনও কোনো বিভাগ নেই</h3>
      <p>একাডেমিক বিভাগ তৈরি করে শুরু করুন</p>
    </div>

    <div v-else class="sections-table">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="s in sections" :key="s.id">
              <td class="section-name"><strong>{{ s.section_name_en || s.section_name }}</strong></td>
              <td class="text-muted">{{ s.section_bn || '-' }}</td>
        ...
        <form @submit.prevent="saveSection" class="modal-body">
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <input v-model="form.section_name" class="form-control" placeholder="Section A" />
            </div>
            <div class="form-group">
              <label>বিভাগের নাম (বাংলা)</label>
...
async function load() {
  ...
  try {
    ...
    const res = await api.get(`/settings/sections?${params}`)
    sections.value = res.data?.data?.data || []
...
async function saveSection() {
  ...
  try {
    const payload: any = {
      ...
      section_bn: form.section_bn,
      capacity: form.capacity,
      status: form.status,
    }

    if (isEditing.value && currentId.value) {
      await api.put(`/settings/sections/${currentId.value}`, payload)
    } else {
      await api.post('/settings/sections', payload)
    }
...
.sections-table { background: white; border: 1px solid var(--color-border-light); border-radius: 14px; overflow: hidden; }
...
.section-name { font-weight: 600; color: var(--color-text); }
