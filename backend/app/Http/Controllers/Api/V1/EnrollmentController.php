...
class EnrollmentController extends ApiController
{
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            ->when($request->filled('is_active'), fn($q) => $q->whereIn('status', filter_var($request->input('is_active'), FILTER_VALIDATE_BOOLEAN) ? ['active', 'enrolled', 'approved'] : ['inactive', 'dropped', 'rejected']))
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $allowedColumns = ['tenant_id', 'student_id', 'class_id', 'section_id', 'session_id', 'enrollment_number', 'enrollment_date', 'status', 'promotion_date', 'promoted_to_class', 'promoted_to_section'];
            $insertData = array_intersect_key($data, array_flip($allowedColumns));

            $enrollment = Enrollment::create($insertData);
            ...
            return $this->errorResponse('নাম নিবন্ধন করতে সমস্যা হয়েছে: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        try {
            ...
            $allowedColumns = ['class_id', 'session_id', 'section_id', 'enrollment_date', 'status', 'promotion_date', 'promoted_to_class', 'promoted_to_section'];
            $updateData = array_intersect_key($validator->validated(), array_flip($allowedColumns));
            ...
            Log::error('Enrollment update error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return $this->errorResponse('নাম নিবন্ধন আপডেট করতে সমস্যা হয়েছে: ' . $e->getMessage(), 500);
        }
    }
