<template>
  <div class="create-page-container">
    ...
    <form @submit.prevent="handleSubmit" class="create-card">
      <div class="form-section">
        <h3 class="section-title">বরাদ্দ সংক্রান্ত বিবরণ</h3>
        <p class="section-desc">শিক্ষক, শ্রেণি এবং বিষয় নির্বাচন করে দায়িত্ব নির্ধারণ করুন</p>
      </div>

      <div class="form-group">
        <label class="form-label">শিক্ষক <span class="required-star">*</span></label>
        <select v-model="form.teacher_id" class="form-control form-select" :disabled="loading || teachers.length === 0" required>
        ...
        <small v-if="teachers.length === 0" class="text-muted">কোনো শিক্ষক তালিকাভুক্ত নেই</small>
      </div>

      <div class="form-row form-row-2">
        <div class="form-group">
          <label class="form-label">শ্রেণি <span class="required-star">*</span></label>
          <select v-model="form.class_id" class="form-control form-select" :disabled="loading || classes.length === 0" @change="loadSections" required>
        ...
        <div class="form-group">
          ...
          <select v-model="form.section_id" class="form-control form-select" :disabled="loading">
      ...
      <div class="form-row form-row-2">
        <div class="form-group">
          ...
          <select v-model="form.subject_id" class="form-control form-select" :disabled="loading || subjects.length === 0">
        ...
        <div class="form-group">
          ...
          <select v-model="form.status" class="form-control form-select" :disabled="loading">
      ...
      <div class="form-group">
        ...
        <input v-model="form.topic_bn" type="text" class="form-control" placeholder="যেমন: আল-কুরআনের প্রথম পারা" :disabled="loading" />
      </div>
      ...
      <div class="form-row form-row-2">
        ...
        <div class="form-group">
          ...
          <input v-model="form.end_date" type="date" class="form-control" :disabled="loading" />
        </div>
      </div>

      <label class="checkbox-label">
        <input type="checkbox" v-model="form.is_active" :disabled="loading" />
        <span>বরাদ্দটি সক্রিয় রাখুন</span>
      </label>
...
.header-left h1 { margin: 0.5rem 0 0; font-family: 'Noto Sans Bengali', sans-serif; font-size: 1.35rem; }
.back-link { display: inline-flex; align-items: center; gap: 0.35rem; color: var(--color-primary); text-decoration: none; font-family: 'Noto Sans Bengali', sans-serif; font-weight: 500; }
.btn { padding: 0.75rem 1.5rem; border-radius: 8px; font-size: 0.95rem; font-weight: 600; cursor: pointer; border: none; font-family: 'Noto Sans Bengali', sans-serif; }
...
.alert { padding: 0.75rem 1rem; border-radius: 8px; font-family: 'Noto Sans Bengali', sans-serif; margin-bottom: 1rem; }
.alert-error { background: #fce4e4; color: var(--color-error); }
.alert-success { background: #e8f5e9; color: var(--color-success); }
.text-muted { color: var(--color-text-light); font-family: 'Noto Sans Bengali', sans-serif; font-size: 0.825rem; margin-top: 0.25rem; display: block; }
</style>
